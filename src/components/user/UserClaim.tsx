...
import { ClaimAction, claimModel, UserClaimState } from './UserClaimTypes';
...
const initialState:UserClaimState = {
    ...
    claims:[],
    claim:{ label:null , value:null}
}

const reducer =(state:UserClaimState,action:ClaimAction) => {
    switch(action.type){
...
interface UserClaimProps{
    userClaims:Array<claimModel>
}

const UserClaim = ({userClaims}:UserClaimProps) => {
    ...
    const onHide = (name:any,save:boolean) => {
        dispatch({type: name ,isOpen:false })

        if(name==="delete-claim-dialog" && save === true){
            dispatch({type:'claims-list-update',claims: state.claims.filter(item=> item.label !== state.claim["label"])})
        }

        //edit operation
        if(name==="edit-claim-dialog" && save === true){
            dispatch({type:'claims-list-update',claims: state.claims.map((claim)=>
            claim.label === state.claim["label"]? {...claim,value:state.claim["value"]} : {...claim})})
        }
        ...
        dispatch({type:'claim-user-update',claim:{ label:null,value:null }})
    }
    ...
    const oprationAddTemplate = ( rowData:claimModel ) => {
        return <Button icon="pi pi-user-edit" onClick={() => modifyClaim(rowData) }/>
    }

    const modifyClaim = ( rowData:claimModel ) => {
        //console.log(rowData)
    ...
    //Delete Opertaions
    const oprationDeleteTemplate = ( rowData:claimModel ) => {
        return <Button icon="pi pi-user-minus" onClick={() => deleteClaim(rowData)}/>
    }

    const deleteClaim = ( rowData:claimModel ) => {
        dispatch({type:'claim-user-update',claim:rowData})
        onClick('delete-claim-dialog')
    }

    //Dialog change of UserClaim for add/Edit
    const handleClaimValueChange = ( event:any ) =>{
        dispatch({type:'claim-user-update',claim: {...state.claim,value:event.target.value}})
    }
    const handleClaimTypeChange = ( event:any ) =>{
        dispatch({type:'claim-user-update',claim: {...state.claim,label:event.target.value}})
    }
